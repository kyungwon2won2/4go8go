...
public class ChatMessageController {
    ...
    public void enter(@Payload ChatMessageDto message, SimpMessageHeaderAccessor headerAccessor) {
        ...
        try {
            ...
            // 컨텍스트에서 인증 정보 확인
                ...
                if (auth != null) {
                    email = auth.getName();
                    log.info("컨텍스트에서 사용자 이메일 확인: {}", email);
                }
            }
            ...
            if (email != null) {
                ...
                if (user != null) {
                    ...
                    log.info("사용자 닉네임 설정: {}", user.getNickname());
                    ...
                    if (participant.isPresent()) {
                        ...
                        if (chatParticipant.getHasEntered() == null || !chatParticipant.getHasEntered()) {
                            log.info("첫 입장 메시지 전송: {}", user.getNickname());
                            ...
                            log.info("이미 입장한 사용자입니다. 입장 메시지 전송하지 않음: {}", user.getNickname());
                        }
                    } else {
    ...
    public void sendMessage(@Payload ChatMessageDto message, SimpMessageHeaderAccessor headerAccessor) {
        ...
        try {
            ...
            if (email == null) {
                log.error("사용자 이메일을 확인할 수 없습니다. 메시지 처리 불가");
                return;
            }
            ...
            if (message.getRoomId() != null) {
                ...
                // 메시지 브로드캐스팅
                log.info("메시지 브로드캐스팅: 룸 ID={}, 메시지={}", message.getRoomId(), message);
                messagingTemplate.convertAndSend("/sub/chat/room/" + message.getRoomId(), message);
                
                // 참가자들에게 읽지 않은 메시지 개수 업데이트 알림
                broadcastUnreadCountUpdates(message.getRoomId());
            } else {
                log.error("roomId가 null입니다. 메시지를 저장하고 브로드캐스팅할 수 없습니다.");
            }
        } catch (Exception e) {
    ...
    private void broadcastUnreadCountUpdates(Long roomId) {
        try {
            ...
            log.error("읽지 않은 메시지 업데이트 알림 전송 중 오류 발생", e);
        }
    }
    ...
    public void sendMessageToRoom(@DestinationVariable Long roomId, @Payload ChatMessageDto message) throws JsonProcessingException {
        log.info("룸 ID {} 직접 메시지: {}", roomId, message);
        ...
        try {
            ...
            log.info("직접 전송 메시지 저장 완료: messageId={}", messageId);
            
            // 메시지 브로드캐스팅
            log.info("직접 전송 메시지 브로드캐스팅: 룸 ID={}, 메시지={}", roomId, message);
            messagingTemplate.convertAndSend("/sub/chat/room/" + roomId, message);
        } catch (Exception e) {
            log.error("직접 메시지 전송 중 오류 발생", e);
        }
    }
}